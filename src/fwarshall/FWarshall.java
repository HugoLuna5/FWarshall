...
/*****************************************************************************
 *                     JAVA Program to implement Floyd                       *
 *                      Author : Hugo Luna                                   *
 *                                                                           *                                                                         
 *****************************************************************************/
...
/**
 * Para agregar un nodo solo se tiene que dar click en el lienzo
 * para conectar dos nodos se tiene que arrastrar uno hacia el otro
 * cuado se termine de agregar y conectar nodos, se tiene que dar click en finalizar
 * y por ultimo click en calcular
 **/
...
public class FWarshall extends JFrame implements ActionListener {
    ...
    private JButton compute = new JButton("Calcular");
    private PrimerVentana clickPanel = new PrimerVentana();       //Object of the PrimerVentana class, creates the clickable panel
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            if(e.getSource() == compute) {
                try {
                    ...
                    new MostrarMatriz();              //Creating MostrarMatriz object
                    dispose();                      //disposing current object
                } 
                catch (Exception ex) {    JOptionPane.showMessageDialog(null, "No existe el grafo");                  //Handle exception where there are no edges 
    ...
    class PrimerVentana extends JPanel implements MouseListener, MouseMotionListener {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if(nodeRight(p)) {
                ...
                JOptionPane.showMessageDialog(null, "No puedes agregar vertices aquí");
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if(dragged) {                
                ...
                if(pointRight(pStart) && pointRight(p)) {
                    ...
                    linePoints.add(p);
                    String str = JOptionPane.showInputDialog(null, "Introduce el peso/distancia");
                    weights.add(Integer.parseInt(str));
                    dragged = false;
                    repaint();
                }
                else {
                    JOptionPane.showMessageDialog(null, "No puedes dibujar aquí");
                    dragged = false;
                }
            }       
        }
    ...
    class MostrarMatriz extends JFrame implements ActionListener {
        private PrimerVentana displayPanel = new PrimerVentana();                     //Adding draph panel to the 2ndPanel
        ...
        public MostrarMatriz() {
            ...
            JLabel label = new JLabel("Matriz Adyacencia\n\n");
            ...
            for(int i=0; i<arr.length; i++) {
                for(int j=0; j<arr.length; j++) {
                    if(stepWise[x-1][i][j] != stepWise[x][i][j]) 
                        changes += "\nValor cambiado en (" + (i+1) + "," + (j+1) + ") de " + (stepWise[x-1][i][j] == 99999 ? "∞" : stepWise[x-1][i][j])  + " a " + stepWise[x][i][j]; 
                }
            }
            return changes;
        }
        ...
        public void displayMatrix() {
            ...
            for(int k=1; k<stepWise.length; k++) {
                area.setText(area.getText() + "Matriz para K : " + k + "\n\n       ");
                for(int i=0; i<arr.length; i++) {
                    for(int j=0; j<arr.length; j++) {
